...
class DataCleaning(luigi.Task):
	...
	def requires(self):
		return UploadtoS3_Raw.UploadDirtyData()
	...
	def run(self):
		...
		with self.output().open("w") as f:
			f.write(os.path.expanduser("~") + "\\Team6_ADS_Assignment1\\Luigi\\Logs\\Temp\\Wrangle_Log.txt")
		...
		current_directory = glob.glob(os.path.expanduser("~") + "\\Team6_ADS_Assignment1\\Luigi\\Data\\DirtyData\\*.csv")
		current_file = current_directory[0]
		current_date = current_file[-23:-15]		
		...
		target_directory = os.path.expanduser("~") + "\\Team6_ADS_Assignment1\\Luigi\\Data\\CleanData\\"
		if current_date in datelist:
			if actual_file_clean not in filenames:
				...
				csv_data_df =pd.read_csv(target_directory + actual_file,encoding='ISO-8859-1',
				...
				with open(target_directory + actual_file_clean, 'w') as myfile:
					myfile.write(csv_data_df.to_csv(index=False))
			
				os.unlink(target_directory + actual_file)
				
				print('Data Wrangling process completed for ', current_date,'!!!!!')
				
			else:
				print('Data Wrangling process is already completed for ', current_date,'!!!!!')
				
		else:
			print('File is not present for ', current_date,' on s3!!!!!')
    
	def output(self):
		return luigi.LocalTarget(os.path.expanduser("~") + "\\Team6_ADS_Assignment1\\Luigi\\Logs\\Temp\\Wrangle_Log.txt")
