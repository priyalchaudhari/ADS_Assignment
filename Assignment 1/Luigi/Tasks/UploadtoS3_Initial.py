...
import io
import luigi
import DataIngestion_Initial

class UploadInitialData(luigi.Task):

	def requires(self):
		return DataIngestion_Initial.DownloadInitialDataSet()
	...
	def run(self):
		...
		with self.output().open("w") as f:
			f.write(os.path.expanduser("~") + "\\Team6_ADS_Assignment1\\Luigi\\Logs\\Temp\\UploadInitial_Log.txt")
	...
	def output(self):
		return luigi.LocalTarget(os.path.expanduser("~") + "\\Team6_ADS_Assignment1\\Luigi\\Logs\\Temp\\UploadInitial_Log.txt")